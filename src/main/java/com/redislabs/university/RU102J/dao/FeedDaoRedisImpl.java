...
public class FeedDaoRedisImpl implements FeedDao {
    ...
    public void insert(MeterReading meterReading) {
        ...
        try(Jedis jedis = jedisPool.getResource()){
            //insert into the global feed
            jedis.xadd(RedisSchema.getFeedKey(meterReading.getSiteId()), StreamEntryID.NEW_ENTRY, meterReading.toMap(), globalMaxFeedLength, true);
            //insert to local feed
            jedis.xadd(RedisSchema.getGlobalFeedKey(), StreamEntryID.NEW_ENTRY, meterReading.toMap(), siteMaxFeedLength, true );
        }
        // END Challenge #6
    }
